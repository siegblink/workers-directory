...
export function useWorkerWithDetails(workerId: number | null) {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true)
      const { data, error } = await getWorkerWithDetails(workerId!)
      setWorker(data)
      setError(error)
      setLoading(false)
    }
...
export function useMyBookings(filters: Omit<BookingFilters, 'customer_id'> = {}) {
...
export function useWorkerBookings(
  ...
  const loadBookings = useCallback(async (id: number, newFilters: typeof filters) => {
    setLoading(true)
    const { data, pagination: pag, error } = await getWorkerBookings(id, newFilters)
    setBookings(data)
    setPagination(pag)
    setError(error)
    setLoading(false)
...
export function useIsFavorite(workerId: number | null) {
  ...
  return { isFavorite: isFav, loading, refresh: () => workerId && checkFavorite(workerId) }
}
